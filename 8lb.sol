...
contract rockScissorsPaper { 
    ...
    function movePlayer1(uint id, bytes32 hash1) public { 
    ...
    function movePlayer2(uint id, bytes32 hash2) public { 
    ...
    function decodeHash1(uint id, string memory seed1) public { 
    ...
    function decodeHash2(uint id, string memory seed2) public { 
    ...
    function seeWinner(uint id) public view returns(address) { 
    if (rooms[id].draw == true) return address(0); 
    else return rooms[id].winner; 
} 
    function str_to_bytes_1(string memory seed) internal pure returns(bytes1){ 
        bytes memory seed_b = bytes(seed); 
        return seed_b[0]; 
    } 
}